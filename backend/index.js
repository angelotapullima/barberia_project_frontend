...
app.delete('/api/stations/:id', async (req, res) => {
  ...
  if (barber) {
    return res
      .status(400)
      .json({ error: 'No se puede eliminar la estación porque está asignada a un barbero.' });
  }
...
app.put('/api/services/:id', async (req, res) => {
  ...
  const result = await db.run('UPDATE services SET name = ?, price = ? WHERE id = ?', [
    name,
    price,
    id,
  ]);
...
app.delete('/api/services/:id', async (req, res) => {
  ...
  if (sale) {
    return res
      .status(400)
      .json({ error: 'No se puede eliminar el servicio porque está asociado a una venta.' });
  }
