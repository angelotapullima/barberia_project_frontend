...
class SettingService {
  ...
  public async getSetting(key: string): Promise<string | undefined> {
    const setting = await this.db.get('SELECT setting_value FROM settings WHERE setting_key = ?', key);
  ...
  public async getAllSettings(): Promise<Setting[]> {
    const settings = await this.db.all('SELECT setting_key, setting_value FROM settings');
    return settings;
  }
}
