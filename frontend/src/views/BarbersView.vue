<template>
  <div class="container mx-auto">
    ...
    <div v-if="barberStore.error" class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg">{{ barberStore.error }}</div>

    <div class="bg-white shadow-md rounded-lg">
      <div class="p-4 border-b flex justify-between items-center">
        <h2 class="text-xl font-semibold">Lista de Barberos</h2>
        <button @click="openModal()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Añadir Barbero</button>
      </div>
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Sueldo Base</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="barber in barberStore.barbers" :key="barber.id">
            ...
            <td class="px-6 py-4 text-right text-sm font-medium">
              <button @click="openModal(barber)" class="text-indigo-600 hover:text-indigo-900">Editar</button>
              <button @click="confirmDelete(barber.id)" class="text-red-600 hover:text-red-900 ml-4">Eliminar</button>
            </td>
          </tr>
          <tr v-if="barberStore.barbers.length === 0">
            <td colspan="4" class="px-6 py-4 text-center text-gray-500">No se encontraron barberos.</td>
          </tr>
        </tbody>
      </table>
    </div>
      ...
      <div class="relative mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
        ...
        <form @submit.prevent="handleSubmit">
          <div class="mb-4">
            ...
            <input v-model="currentBarber.name" type="text" id="name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" required>
          </div>
          <div class="mb-4">
            <label for="base_salary" class="block text-sm font-medium text-gray-700">Sueldo Base</label>
            <input v-model.number="currentBarber.base_salary" type="number" id="base_salary" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
          </div>
          <div class="mb-4">
            <label for="station" class="block text-sm font-medium text-gray-700">Estación</label>
            <select v-model="currentBarber.station_id" id="station" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
