<template>
  <div class="container mx-auto p-6">
    ...
    <div v-if="authStore.isLoading" class="text-center text-gray-500">Cargando perfil...</div>
    <div v-if="authStore.error" class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg">
      {{ authStore.error }}
    </div>
    ...
    <div class="bg-white shadow-lg rounded-xl p-8">
      ...
      <form @submit.prevent="handleChangePassword">
        <div class="mb-4">
          <label for="oldPassword" class="block text-sm font-semibold text-gray-700 mb-1">Contraseña Actual:</label>
          <input type="password" id="oldPassword" v-model="oldPassword" class="form-input" required>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-6">
          <label for="confirmNewPassword" class="block text-sm font-semibold text-gray-700 mb-1">Confirmar Nueva Contraseña:</label>
          <input type="password" id="confirmNewPassword" v-model="confirmNewPassword" class="form-input" required>
        </div>

        <div v-if="passwordChangeError" class="p-3 mb-4 text-sm text-red-700 bg-red-100 rounded-lg">
          {{ passwordChangeError }}
        </div>
        <div v-if="passwordChangeSuccess" class="p-3 mb-4 text-sm text-green-700 bg-green-100 rounded-lg">
...
const handleChangePassword = async () => {
  ...
  try {
    ...
    if (!token) {
      passwordChangeError.value = 'No autenticado. Por favor, inicia sesión de nuevo.';
      return;
    }
    ...
    if (error.response && error.response.data && error.response.data.message) {
      ...
      passwordChangeError.value = 'Error al cambiar la contraseña. Inténtalo de nuevo.';
    }
  } finally {
